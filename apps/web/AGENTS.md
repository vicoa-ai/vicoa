...
- Group / section labels in sidebars and panels (a project group, "Today", "Workspace", "Active links") are sentence case in the dashboard sidebar's header style: `text-[0.8rem] font-normal text-muted-foreground` (see the group header in `components/dashboard/sidebar-sessions.tsx`). Never `uppercase`, `tracking-wide` or bold — an all-caps bold label reads as a different product from the rest of the chrome.
