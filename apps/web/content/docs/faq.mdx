...
Yes. Open the session's **⋯** menu (or the **Share** button in its header) and create a link. Anyone with the link gets a live, read-only view of the transcript in their browser, with no account needed. You can limit a link to signed-in Vicoa users, set an expiry, and revoke it at any time. A project's task board can be shared the same way. By default the page shows nothing about who shared it; you can choose to show your name. See [Share Sessions and Boards with a Link](/docs/vicoa-features#share-sessions-and-boards-with-a-link).
