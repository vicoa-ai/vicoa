...
## Can I share a session with someone who doesn't use Vicoa?

Yes. Open the session's **⋯** menu (or the **Share** button in its header) and create a link. Anyone with the link follows the session live in their browser, with no account needed. You can limit a link to signed-in Vicoa users, set an expiry, and revoke it at any time. A whole project can be shared the same way: one link carries its tasks, its sessions, or both. By default the page shows nothing about who shared it; you can choose to show your name and avatar. See [Share Sessions and Projects with a Link](/docs/vicoa-features#share-sessions-and-projects-with-a-link).
