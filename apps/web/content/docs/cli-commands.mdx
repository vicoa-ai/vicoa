...
| `404` on `task get/update/delete` | Use the `VIC-42` identifier or the **full UUID** — the 8-char `ID` column is display-only. Automations still need their full UUID; only `session` commands and `vicoa stop` accept an 8-char prefix. |
