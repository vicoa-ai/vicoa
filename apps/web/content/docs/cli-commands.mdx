...
`create` defaults to `status=backlog`, `priority=none`, and is filed under **No project** unless you pass `--project <PROJECT_ID>`.

| Flag (create / update) | Meaning |
|---|---|
| `--description <text>` | Longer body |
| `--project <PROJECT_ID>` | Project to file under (defaults to No project) |
