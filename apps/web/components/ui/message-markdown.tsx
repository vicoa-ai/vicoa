...
type MessageAgentType = 'claude' | 'codex' | 'opencode';
// unified's plugin-list types, reached through react-markdown so we don't take
// a direct dependency on unified just for a type.
...
const CODE_FENCE_PLACEHOLDER = 'XCODEFENCEPLACEHOLDERX';
const TODO_LINE_REGEX = /([●◐○])([^\n]*)/g;

const transformTodoLine = (line: string) => {
  return line.replace(TODO_LINE_REGEX, (match, symbol, text) => {
  ...
  for (let idx = 0; idx < lines.length; idx += 1) {
    ...
    if (markdownOuterCloseIndex !== -1) {
      ...
      if (
        ...
        output.push(line.replace(/```/g, CODE_FENCE_PLACEHOLDER));
        continue;
      }
...
const restorePlaceholders = (value: string) => value.split(CODE_FENCE_PLACEHOLDER).join('```');
...
// The per-message inputs the `code` and `a` renderers need. They reach them
// through this context rather than closures so that the `components` map below
// can be one module-level object. react-markdown uses each entry as a React
// component *type*: a map rebuilt per render hands React a new type every
// time, which unmounts and remounts the entire message DOM. Besides the cost,
// that yanked the nodes out from under a live text selection — every
// re-render mid-drag moved the anchor back to the container and the
// re-inserted blocks landed inside the selection, so it grew backwards
// through the message.
interface MessageRenderContextValue {
  agentType?: MessageAgentType;
}
const MessageRenderContext = createContext<MessageRenderContextValue>({});
...
const MARKDOWN_COMPONENTS: Components = {
  ...
  code: (props) => {
    ...
    const { agentType } = useContext(MessageRenderContext);
...
const HIGHLIGHT_PLUGIN: Pluggable = [rehypeHighlight, { detect: true, ignoreMissing: true }];

function MessageMarkdownImpl({ children, agentType, highlightQuery }: MessageMarkdownProps) {
  const transformedContent = useMemo(
    () => preprocessContent(formatTaskNotifications(normalizeCommandOutput(children))),
    [children],
  );
  ...
  const renderContext = useMemo(() => ({ agentType }), [agentType]);

  return (
    <div className="font-mono">
      <MessageRenderContext.Provider value={renderContext}>
        <ReactMarkdown
...
// not every existing message in the list. File-link context changes reach the
// `a` renderer directly (it subscribes itself), so they don't re-render — let
// alone remount — the rest of the message.
